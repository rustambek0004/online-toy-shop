...
def addOrder(request, cart_id):
    item_counter = 0
    try:
        caart = Cart.objects.get(id=request.session.get('user_cart_id'))
    except:
        caart = {'notfound': "Products not found!"}
    ...
    return render(request, 'cart/order.html', {'form':form, 'caart':caart})
